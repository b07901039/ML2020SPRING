# problem 3, outer loop使用cosine annealing scheduler lr. inner loop 如proplem 2,更新5次並使用Adagrad
...
lr_list = []
# p2 (inner loop iter = 5 + adagrad) + p3 scheduler
def MAML(model, optimizer, scheduler, x, n_way, k_shot, q_query, loss_fn, inner_train_step = 1, inner_lr = 0.4, train = True):
  """
  Args:
...
if __name__ == '__main__':
    ...
    for epoch in range(max_epoch):
      ...
      for step in tqdm(range(len(train_loader) // (meta_batch_size))): # 這裡的 step 是一次 meta-gradinet update step
        ...
        meta_loss, acc = MAML(meta_model, optimizer, scheduler, x, n_way, k_shot, q_query, loss_fn, 
                              inner_train_step, inner_lr, train = True)
      ...
      for eval_step in tqdm(range(len(val_loader) // (eval_batches))):
        ...
        _, acc = MAML(meta_model, optimizer, scheduler, x, n_way, k_shot, q_query, loss_fn, inner_train_step = 5, inner_lr = inner_lr, train = False) # testing時，我們更新三次 inner-step
        val_acc.append(acc)
      print("  Validation accuracy: ", np.mean(val_acc))
      val_accs.append(val_acc)

    # np.save("./val_acc.npy", val_accs)
    ...
    for test_step in tqdm(range(len(test_loader) // (test_batches))):
      ...
      _, acc = MAML(meta_model, optimizer, scheduler, x, n_way, k_shot, q_query, loss_fn, inner_train_step = 5, inner_lr = inner_lr, train = False) # testing 時，我們更新三次 inner-step
      test_acc.append(acc)
    print("  Testing accuracy: ", np.mean(test_acc))
    # np.save("./lr_sgd.npy", lr_list)