# problem 4 (train), reptile, inner loop 更新5次 使用adagrad
...
if __name__ == '__main__':
    ...
    for epoch in range(max_epoch):
      ...
      for step in tqdm(range(len(train_loader) // (meta_batch_size))): # 這裡的 step 是一次 meta-gradinet update step
        ...
        meta_loss, acc = MAML(meta_model, x, n_way, k_shot, q_query, loss_fn, 
                              inner_train_step, inner_lr, meta_lr=meta_lr, train = True)
      ...
      for eval_step in tqdm(range(len(val_loader) // (eval_batches))):
        ...
        _, acc = MAML(meta_model, x, n_way, k_shot, q_query, loss_fn, 
                    inner_train_step = 5, inner_lr = inner_lr, meta_lr=meta_lr, train = False) # testing時，我們更新三次 inner-step
        val_acc.append(acc)
      print("  Validation accuracy: ", np.mean(val_acc))
      val_accs.append(val_acc)

    # np.save("./val_acc.npy", val_accs)
    torch.save(meta_model.state_dict(), os.path.join(model_dir, "model_b07901039.bin"))
