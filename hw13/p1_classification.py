# problem 1, 一階maml (更改create_graph = False)
...
def MAML(model, optimizer, x, n_way, k_shot, q_query, loss_fn, inner_train_step = 1, inner_lr = 0.4, train = True):
  ...
  for meta_batch in x:
    ...
    for inner_step in range(inner_train_step): # 這個 for loop 是 Algorithm2 的 line 7~8
      ...
      grads = torch.autograd.grad(loss, fast_weights.values(), create_graph = False) # 這裡是要計算出 loss 對 θ 的微分 (∇loss) ## 更改!! 
...
if __name__ == '__main__':
    ...
    k_shot = 1
    q_query = 1
    inner_train_step = 1 # original case = 1, p2 case = 5
    inner_lr = 0.4 # original case = 0.4, p2 case = 
    meta_lr = 0.001
    meta_batch_size = 32
    max_epoch = 40
    ...
    for epoch in range(max_epoch):
      ...
      for eval_step in tqdm(range(len(val_loader) // (eval_batches))):
        ...
        _, acc = MAML(meta_model, optimizer, x, n_way, k_shot, q_query, loss_fn, inner_train_step = 5, inner_lr = inner_lr, train = False) # testing時，我們更新三次 inner-step
        val_acc.append(acc)
    ...
    for test_step in tqdm(range(len(test_loader) // (test_batches))):
      ...
      _, acc = MAML(meta_model, optimizer, x, n_way, k_shot, q_query, loss_fn, inner_train_step = 5, inner_lr = inner_lr, train = False) # testing 時，我們更新三次 inner-step
      test_acc.append(acc)
